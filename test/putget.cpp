...
//            g.create_window(i, device_local_window,device_remote_window, k_DEVICE_WINDOW_SIZE) ;
            g.stream_put(i, stream,0, 0,k_DEVICE_WINDOW_SIZE ) ;
            g.stream_wait_put_complete(i, stream) ;
    ...
    for ( int i=0; i<size; i+=1)
      {
        if ( i != rank )
          {
            g.stream_get(i, stream,0, 0,k_DEVICE_WINDOW_SIZE) ;
            g.stream_wait_get_complete(i, stream) ;
            CUDA_CHECK(cudaStreamSynchronize(stream)) ;
